...
function validateComment(comment: string, code: string): boolean {
  ...
  const cleanedComment = comment.trim().replace(/^```[\s\S]*?\n/, "").replace(/\n```$/, "");
  ...
  return (
    ...
    !/^\s*(function|def|class|public\s+|private\s+|const\s+\w+\s*=\s*\(|let\s+\w+\s*=\s*\(|var\s+\w+\s*=\s*\(|\w+\s*=\s*lambda)/.test(cleanedComment) // Detects common function/class definitions
  );
}
